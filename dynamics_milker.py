...
data_folder_name = '/data/Lior/lst/2020/2020_01_13/'


save_folder_name = data_folder_name
file_wild_card = data_folder_name + 'TLP1_Thy2_GCaMP7_FITC_mouse_930nm_512l_1slow_3p5x_zoom_+200um_height_FOV1_TAG_OFF_SYNC_ON_+20um_deep_comparable_to_200um_???.zarr'
#file_wild_card = data_folder_name + 'TLP1_Thy2_GCaMP7_FITC_mouse_930nm_512l_1slow_3p5x_zoom_+200um_height_FOV1_TAG_OFF_SYNC_ON_+20um_deep_comparable_to_200um_099.zarr'

#file_wild_card = data_folder_name + 'TLP1_Thy2_GCaMP7_FITC_mouse_930nm_512l_1slow_3p5x_zoom_+200um_height_FOV1_TAG_OFF*.zarr'
#data_file_name = 'TLP1_Thy2_GCaMP7_FITC_mouse_930nm_512l_1slow_3p5x_zoom_+200um_height_FOV1_TAG_OFF_SYNC_ON_+20um_deep_comparable_to_200um_099.zarr'
#data_file_name = 'TLP1_Thy2_GCaMP7_FITC_mouse_930nm_512l_1slow_3p5x_zoom_+100um_height_FOV1_035.zarr'
#data_file_name = 'TLP1_Thy2_GCaMP7_FITC_mouse_930nm_1024l_1slow_3p5x_zoom_+100um_height_FOV1_159.zarr'
#data_file_name = 'TLP1_Thy2_GCaMP7_FITC_mouse_930nm_512l_1slow_3p5x_zoom_+100um_height_FOV1_038.zarr'
#data_file_name = 'TLP1_Thy2_GCaMP7_FITC_mouse_930nm_512l_1slow_3p5x_zoom_+200um_height_FOV1_TAG_OFF_SYNC_ON_+20um_deep_comparable_to_200um_102.zarr'
#data_file_name = 'TLP1_Thy2_GCaMP7_FITC_mouse_930nm_1024l_1slow_3p5x_zoom_+100um_height_FOV1_061.zarr' #'milkytest.zarr'
#data_file_name = 'TLP1_Thy2_GCaMP7_FITC_mouse_930nm_256l_1slow_3p5x_zoom_+100um_height_FOV1_169.zarr'
#data_file_name = 'TLP1_Thy2_GCaMP7_FITC_mouse_930nm_512l_1slow_3p5x_zoom_+200um_height_FOV1_TAG_OFF_SYNC_ON_+20um_deep_comparable_to_200um_096.zarr'
...
#VOI_file_name = 'TLP1_Thy2_GCaMP7_FITC_mouse_930nm_256l_1slow_3p5x_zoom_+100um_height_max_power_FOV1_VISIBLE_N_summed_bc_EURONS_327_summed_bc_rs_2048x2048_3d_gauss_3x3x1_squared_Overlay_Elements.csv'
VOI_file_name = data_folder_name + 'Overlay Elements of TLP1_Thy2_GCaMP7_FITC_mouse_930nm_1024l_1slow_3p5x_zoom_+100um_heigh_summed_bc_t_FOV1_062_summed_bc_top256_slices_rs2048x2048_bc_3d_gauss_3x3x4.csv'

#axial_feature_radius = int(12) # grabbing 12 slices above and below the center of each feature of interest
axial_feature_radius = int(15) # grabbing 15 slices above and below the center of each feature of interest


"""
sample_file_name = data_folder_name + data_file_name

save_fn = save_folder_name + data_file_name[:-5] + '_vascular_traces_tz_debug.zarr'
...
print('data shape[0] is:') # debugging
print(data_shape[0])


number_of_cropped_frames = 1500 # int(data_shape[0]) # 25
...
def fiji_voi_reader(file_name, axial_feature_radius, num_slices, old_VOI='False'):
 ...
 with open(file_name, newline='') as f:
  ...
  for counter, row in enumerate(reader):
   if counter == 0: # skip header of csv file
    continue
   if old_VOI=='True':
     column_index = 3 # old FIJI VOI files had one column less
   else:
     column_index = 5

   print('counter = ' + str(counter))
   specific_voi_coordinates[0] = int(row[column_index+1]) - 1 # FIJI indices run from 1 rather than 0
   specific_voi_coordinates[1] = specific_voi_coordinates[0] + int(row[column_index+3])
   specific_voi_coordinates[2] = int(row[column_index]) - 1 # FIJI indices run from 1 rather than 0
   specific_voi_coordinates[3] = specific_voi_coordinates[2] + int(row[column_index+2])
   specific_voi_coordinates[4] = np.maximum(int(row[-2]) - 1 - int(axial_feature_radius), 0) # no need to modify column indices from end of table
   specific_voi_coordinates[5] = np.minimum(int(row[-2]) - 1 + int(axial_feature_radius), num_slices) # no need to modify column indices from end of table
...
def milkman(dask_zarr_handle, coor_array, rap):
...
feature_dynamics_tz = milkman(dz, coordinate_array, reciprocal_axial_intensity_profile)
...
def milk_many_files(volumetric_fname, coordinate_fname, rap_fname, biological_data_type):
#def milk_many_files(file_wc, coordinate_array, reciprocal_axial_intensity_profile):
        ...
        save_fn = volumetric_fname[:-5] + '_' + str(biological_data_type) + '_traces_tz.zarr'
        ...
        save_dataset(save_fn,'vascular_dynamics_tz_' + str(number_of_cropped_frames), feature_dynamics_tz)
        save_dataset(save_fn,'vascular_dynamics_' + str(number_of_cropped_frames), np.sum(feature_dynamics_tz,axis=1))

#milk_many_files(file_wild_card, coordinate_array, reciprocal_axial_intensity_profile)

biological_data_type = 'vascular' # Are you milking vessel segments, neuronal cell bodies, or both?
all_files = [(fname, coordinate_fname, axial_intensity_file_name, biological_data_type) for fname in glob.glob(file_wild_card)]
