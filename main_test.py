...
def main(settings: Settings):
    ...
    datapath = '/pblab/pblab/Lior/2PMVascSeg/Test_trained_model/t'
    matfiles = []
    valid_extensions = tuple(['.mat', '.tif', '.tiff', '.zarr'])
    # Filenames with full path
    for f in listdir(datapath):
        if isfile(join(datapath, f)) and f.endswith(valid_extensions):
        ...
        for testmouse in test_list:
            ...
            for test_batch in batch_generator([testmouse], batch_size):
                ...
                for patch_i in range(batch_size):
                    y_te_cube_predicted[tuple(test_batch.get_index_list()[patch_i])] = y_pred_p[patch_i, ..., 0]
