...
class Mouse:
    ...
    def __init__(self, mouse_n: str, *, img_num: int, is_test: bool, preprocess: bool = False,
        ...
        for aug_i in range(img_num):
            ...
            # read data
            if mouse_n.endswith(tuple(['.tif', '.tiff'])):
                temp_o = tf.imread(mouse_n)
            elif mouse_n.endswith('.zarr'):
                z = zarr.open(mouse_n,mode='r')
                temp_o = np.array(z['summed_bc_stack'])         
            else:
                try:                    
                    mat_contents = sio.loadmat(mouse_n) # only works with small mat files (v7.2 or earlier)
                except:
                    mat_contents = mat73.loadmat(mouse_n) # only works with HDF5-based mat files (v7.3 and up)
                temp_o = mat_contents['cube'] #arrangement is (x,y,z) in order
                
            if(temp_o.shape[2]<128): # avoiding error if the number of z-slices is lower than 128:
                temp_o = np.concatenate(    (temp_o,    np.zeros((temp_o.shape[0],temp_o.shape[1],128-temp_o.shape[2]), dtype='uint16')  ) , axis=2) 
            #print('The data type of the input array is: ' + str(temp_o.dtype) )
            #print(temp_o.shape)   
...
class Patch:
    ...
    def get_original_image(self):
        return self._mouse.get_original_image(self._img_n)[tuple(self._index)]

    def get_ground_truth(self):
        return self._mouse.get_ground_truth(self._img_n)[tuple(self._index)]
...
def save_3d_tiff(path_prefix: str, img_dict: Dict[str, np.ndarray]):
    ...
    for name in img_dict:
        ...
        if img.min() >= 0 and img.max() <= 1:
            ...
            print('minimal image value is: ' + str(img.min()))
            print('maximal image value is: ' + str(img.max()))
            print('The image data is not in 0~1!')
