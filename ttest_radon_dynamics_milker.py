# Temporary sanity check for the code below. Checks its output for two dummy vessels - one straight and the other rotated - with a comparable width of ~70 pixels. 

# Milk temporal dynamics from branching vessels in previously segemented volumes of interest, preparing them for downstream analysis
...
number_of_cropped_frames = 1000 # 1900 # np.minimum(900, int(data_shape[0])) # 3000 # int(data_shape[0]) # 25
...
default_pixel_to_micron_ratio = float(2048/543)
...
VOI_file_name = data_folder_name + 'Overlay Elements of TLP1_Thy2_GCaMP7_FITC_mouse_930nm_1024l_1slow_3p5x_zoom_+100um_heigh_summed_bc_t_FOV1_062_summed_bc_top256_slices_rs2048x2048_bc_RoiSet_branching_vessels_only.csv'

axial_feature_radius = int(20) # grabbing 20 slices above and below the center of each feature of interest
...
def test_milkman(vascular_image, radon_vascular_angles, minimal_number_of_frames):


    ###############################
    num_features = int(1) # carved in rock
    line_binning_factor = int(1)
    ...
    for feature_number in range(num_features):
        ...
        num_cropped_slices = 1


        downscaled_feature = downscale_local_mean(vascular_image, downscale_factors)
        ...
        inflated_image = rescale(downscaled_feature_lp_mean,(line_binning_factor,1))
        time_collapsed_radon_profile = radon(np.squeeze(inflated_image), theta=radon_vascular_angles[feature_number], circle=False, preserve_range=False)
        plt.plot(time_collapsed_radon_profile)
        plt.show()      
        global_radon_threshold[feature_number] = calc_global_radon_threshold(time_collapsed_radon_profile) # using the mean downscaled_feature_lp for a smoother, time-invariant radon threshold imposed on all frames
...
for filenum in range(2):
    ...
    save_fn =  fn.replace(".tif", "_milked.zarr")
    hdf5_file_name = save_fn.replace("zarr", "hdf5")

    test_vessel = np.array(imread(fn))

    print(f'shape of test vessel is {test_vessel.shape}')

    feature_shape = np.sum(test_vessel,axis=0)
    num_frames_in_data = test_vessel.shape[0]

    feature_number = int(0)

    minimal_number_of_frames = np.minimum(number_of_cropped_frames, num_frames_in_data) # accounting for datasets whose number of frames is smaller than number_of_cropped_frames

    save_dataset(save_fn,'cropped_feature_shape_' + str(feature_number), feature_shape )

    vascular_angles_in_radon_space = np.ones((1,1))

    #radon_image = radon(feature_shape, theta=None, circle=False, preserve_range=False)
    radon_image = radon(feature_shape, theta=None, circle=True, preserve_range=False)
    ...
    vascular_angles_in_radon_space[feature_number] = find_vascular_angle(radon_image) 
    ...
    pixel_to_micron_ratio = 1
    #downscaled_feature_lp, time_varying_diameter_in_microns, photon_count_lp = milkman(dz, coordinate_array, ds_factors, reciprocal_axial_intensity_profile, vascular_angles_in_radon_space, minimal_number_of_frames)
    downscaled_feature_lp, time_varying_diameter_in_microns, photon_count_lp = test_milkman(test_vessel[:minimal_number_of_frames,:], vascular_angles_in_radon_space, minimal_number_of_frames)

    plt.plot(time_varying_diameter_in_microns)
    plt.plot(np.divide(photon_count_lp,np.max(photon_count_lp))*(2+np.max(time_varying_diameter_in_microns)))
    plt.show()
