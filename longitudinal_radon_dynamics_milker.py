...
number_of_cropped_frames = 49500 # np.minimum(900, int(data_shape[0])) # 3000 # int(data_shape[0]) # 25
...
axial_feature_radius = int(5) # int(20) # grabbing 20 slices above and below the center of each feature of interest



fn_list = glob.glob(file_wild_card)
name_sorted_fn_list = fn_list.sort() # sort the files by their temporal order, preserved in the file name
fn_list = name_sorted_fn_list
...
coordinate_fname = VOI_file_name[:-4] + '_sanitized_debuggg.zarr'
...
def milkman(dask_zarr_handle, coor_array, downscale_factors, rap, radon_vascular_angles, minimal_number_of_frames):
    ...
    for feature_number in range(num_features):
        ...
        if np.isnan(global_radon_threshold[feature_number]):
          plt.imshow(np.squeeze(inflated_image))
          plt.title(f'estimated angle is {radon_vascular_angles[feature_number]}')
          plt.show()
        ...
        if np.isnan(global_radon_threshold[feature_number]):
          time_varying_radon_diameter[:, feature_number] = np.nan
        else:
...
def extract_all_radon_angles(angle_fname, coordinate_fname):
  ...
  save_fn = angle_fname.replace('.tif','_extracted_angles.zarr')
  hdf5_file_name = angle_fname.replace('.tif','_extracted_angles.hdf5')
  ...
  for feature_number in range(num_features):
    feature_shape = np.sum(best_image[(slice(coordinate_array[feature_number,0],coordinate_array[feature_number,1]), slice(coordinate_array[feature_number,2], coordinate_array[feature_number,3]), slice(coordinate_array[feature_number,4], coordinate_array[feature_number,5]))], axis=2)
...
def milk_many_files(volumetric_fname, coordinate_fname, ds_factors, rap_fname, biological_data_type, vascular_angles_in_radon_space):
...
#def milk_many_files(file_wc, coordinate_array, reciprocal_axial_intensity_profile):
        ...
        save_fn = volumetric_fname[:-5] + '_' + str(biological_data_type) + '_' + str(lowpass_frequency) + '_Hz_lowpass_filtering_radon_debuggg.zarr'
        ...
        ''' # this section has become been moved to function extract_all_radon_angles which acts on the time collapsed imagery across the entire dataset: 
        vascular_angles_in_radon_space = np.zeros((num_features,1))
        ...
        # end of section moved to extract_all_radon_angles'''
