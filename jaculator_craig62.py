...
def get_stacks(file_wc, first_slice=0, last_slice=-1):
    fn_list = glob.glob(file_wc)
    num_data_files = len(fn_list)
    ...
    num_lines, num_columns, num_slices = z['Summed Stack']['Channel 1'][:,:,first_slice:last_slice].shape
    ...
    sfn = fname.replace('.zarr','_jaculated_1000_two_sides_medial.zarr')
...
if __name__=='__main__':
    ...
    #file_wild_card = fld_name + 'TLP1_Thy2_GCaMP7_FITC_mouse_930nm_1024l_1slow_3p5x_zoom_+100um_height_FOV1_062.zarr'
    file_wild_card = fld_name + 'TLP1_Thy2_GCaMP7_FITC_mouse_930nm_512l_1slow_3p5x_zoom_+100um_height_FOV1_034.zarr'

    reference_volume_fn = fld_name + 'ref_2020_01_13_206_to_305_slices.tif'
    ...
    summed_stack, save_fn = get_stacks(file_wild_card, first_slice=first_slice,  last_slice=last_slice)
 ...
 #   for time_iter_num in range(8,num_iterations):
    for time_iter_num in range(num_iterations):
        for y_iter_num in range(num_y_iterations):
            ...
            vaseline = gaussian(z['Full Stack']['Channel 1'][slice(time_iter_num*time_step_size,(time_iter_num+1)*time_step_size),:,slice(y_iter_num*y_step_size, (y_iter_num+1)*y_step_size),slice(first_slice,last_slice)], sigma=(2,3,12,15))
    ...
    h5_save_dataset(h5_save_fn, '100_medial_slices',holy_moly)
    #h5_save_dataset(h5_save_fn, '100_medial_slices_side_view',side_dish)

    save_dataset(save_fn,'100_medial_slices_'+str(num_cropped_frames)+'_frames', holy_moly)
    #save_dataset(save_fn, '100_medial_slices_side_view_side_view_'+str(num_cropped_frames)+'_frames',side_dish)
